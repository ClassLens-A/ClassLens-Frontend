...
export function StudentEnrollmentsPage({ token }: StudentEnrollmentsPageProps) {
  ...
  const fetchEnrollments = async () => {
    ...
    try {
      const response = await fetch("http://127.0.0.1:8000/api/admin/student-enrollments/", {
  ...
  const handleDelete = async (id: number) => {
    ...
    try {
      const response = await fetch(`http://127.0.0.1:8000/api/admin/student-enrollments/${id}/`, {
        method: "DELETE",
        headers: { Authorization: `Bearer ${token}` },
      })
    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-muted-foreground mt-1">Manage which students are enrolled in which subjects</p>
        </div>
        ...
        <div className="overflow-x-auto">
          <table className="w-full">
            <thead className="border-b border-border">
              <tr>
                <th className="text-left p-6 font-semibold text-foreground">Student PRN</th>
              ...
              {loading ? (
                ...
                filteredEnrollments.map((enrollment) => (
                  <tr key={enrollment.id} className="border-b border-border hover:bg-muted/50 transition">
                    <td className="p-6 font-medium text-foreground">{enrollment.student_prn}</td>
                    <td className="p-6 text-muted-foreground">{enrollment.subject_code}</td>
                    <td className="p-6 text-muted-foreground">{enrollment.subject_name}</td>
                    <td className="p-6">
