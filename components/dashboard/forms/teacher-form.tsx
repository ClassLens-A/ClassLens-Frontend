...
export function TeacherForm({ token, teacher, onClose }: TeacherFormProps) {
    ...
    const fetchDepts = async () => {
      ...
      try {
        ...
        if (
          ...
          teacher &&
          teacher.department &&
          typeof teacher.department === "number"
        ) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const url = teacher
        ? `${process.env.NEXT_PUBLIC_BACKEND_URL}/api/admin/teachers/${teacher.id}/`
      ...
      if (!response.ok) {
        ...
        const detail =
          data.detail || JSON.stringify(data) || "Failed to save teacher";
        setError(detail);
        return;
      }
          ...
          <div>
            ...
            {deptsLoading ? (
              ...
              <div className="p-2 text-sm text-destructive">
                Failed to load departments: {deptsError}
              </div>
            ) : (
              <select
